...
public final class AuctionController {
	...
	static private List<Auction> list;
	...
	static public void toApply(BoardInstance bi, PcInstance pc, int uid){
		...
		if(pc.getClassType()!=Lineage.LINEAGE_CLASS_ROYAL || pc.getClanId()==0){
	...
	static public void toApplyFinal(BoardInstance bi, PcInstance pc, int uid, int count){
		...
		if(pc.getInventory().isAden(count, true)){
			...
			if(a.getBidder()!=null && a.getBidder().length()>0)
				LetterController.toLetter(bi, a.getBidder(), "알려드립니다.", String.format("귀하가 제시하신 금액보다 더 많은 금액을 제시하신 분이 나타나 안타깝게도 입찰에 실패하게 되었습니다.\r\n귀하가 경매를 위해 예치하셨던 %d아데나를 반환해 드립니다.\r\n감사합니다.", a.getPrice()), a.getPrice());
			// 입찰자 정보 변경.
	 ...
	 * @param a
	 */
	static private void update(Connection con, Auction a){
		PreparedStatement st = null;
		try {
					...
					if(a.getBidder()!=null && a.getBidder().length()>0){
						...
						if( isAuction(con, a.getBidder()) ){
							...
							if(a.getAgent()!=null && a.getAgent().length()>0){
								...
								LetterController.toLetter(null, a.getAgent(), "알려드립니다.", String.format("귀하가 소유하고 있던 집이 최종가격 %d아데나에 낙찰되었습니다.\n수수료 10%%를 뺀 나머지 금액 %d아데나를 드리겠습니다.\n감사합니다.", a.getPrice(), aden), aden);
							}
							// 축하 편지지 발송.
							LetterController.toLetter(null, a.getBidder(), "축하합니다.", String.format("축하합니다.\n귀하께서 참여하신 경매는 최종가격 %d아데나의 가격으로 낙찰되었습니다.\n이제 귀하가 구매하신 집으로 가서 여러 가지 시설을 이용하실 수 있습니다.\n감사합니다.", a.getPrice()), 0);
							// 아지트 소유권 이전.
...
//----------------------------------------------------------------
							...
							LetterController.toLetter(null, a.getBidder(), "알려드립니다.", "귀하가 참여하신 경매에 성공하셨으나 현재 집을 소유할 수 없는 상태에 있습니다.따라서 부득이하게 귀하의 소유권을 박탈하니, 양해해 주시기 바랍니다.\n감사합니다.", 0);
						}
					}
					...
					if(a.getAgent()!=null && a.getAgent().length()>0){
						if( isAuction(con, a.getAgent()) ){
							// 안내 편지지 발송.
							LetterController.toLetter(null, a.getAgent(), "알려드립니다.", "귀하께서 신청하신 경매는 경매 기간동안 제시하신 금액 이상으로 지불의사를 밝히신 분이 나타나지 않아 결국 취소되었습니다.\n따라서, 소유권이 귀하에게로 돌아갔음을 알려드립니다.\n감사합니다.", 0);
							// 정보 초기화.
							a.setPrice(0);
							a.setBidder("");
							a.setSell(false);
...
//----------------------------------------------------------------
					...
					a.setBidder("");
					// 경매 시간을 5일로 연장하기
					a.setDay(System.currentTimeMillis()+(1000*60*60*24*5));
				}
			}
		} catch (Exception e) {
