...
public final class ShipController {
	...
	// 선착장에서 내릴 타이밍 분을 지정.
	static private int getoff_minute = 59; // 원래 59
	...
	static private void onTalkingIsland(int hour, int minute){
		...
		if(hour==7 || hour==11 || hour==15 || hour==19){
			if(!isUpdateIsland){
				...
				World.set_map(32732, 32796, 5, 127); //이동 중인 배안
				World.set_map(32733, 32796, 5, 127); //이동 중인 배안
				World.set_map(32734, 32796, 5, 127); //이동 중인 배안
				isUpdateIsland = true;
			}
		}else{
			if(isUpdateIsland){
				...
				World.set_map(32732, 32796, 5, 0); //이동 중인 배안
				World.set_map(32733, 32796, 5, 0); //이동 중인 배안
				World.set_map(32734, 32796, 5, 0); //이동 중인 배안
	...
	static private void toTeleport(int ship_map, int item_nameid, int x, int y, int map){
		for( PcInstance pc : World.getPcList() ){
			if(pc.getMap()==ship_map){
				// 표가 잇으면 제거
				pc.getInventory().remove( pc.getInventory().findDbNameId(item_nameid), true );
				// 선착장으로 텔레포트
