...
public final class ColosseumController {
	...
	static private void toKent(){
		...
		c.setTimeStart(new int[]{1, 7, 13, 19});	// 콜롯세움이 활성화될 리니지월드 시간. 시단위
		...
		c.setJoinMaxLevel(15);			// 참가 가능 최고 레벨
	...
	static private void toTalkingIsland(){
		...
		c.setJoinMaxLevel(25);			// 참가 가능 최고 레벨
		c.setJoinTeleport(false);		// 텔레포트
		c.setJoinResurrection(true);	// 부활
		c.setJoinPotion(true);			// 포션 사용
		c.setJoinHp(true);				// 자연 HP 변화
								...
								if(c.list_spawn_idx<list_spawn_max && c.timer_ment_cnt++%spawn_sleep == 0){
									...
									for(int i=0 ; i<count ; ++i){
										...
										if(mi != null){
											...
											MonsterSpawnlistDatabase.toSpawnMonster(mi, World.get_map(c.getMap()), true, c.getX(), c.getY(), c.getMap(), Lineage.SEARCH_MONSTER_TARGET_LOCATION, 0, false, true);
										}
									}
									// 값 갱신.
									c.list_spawn_idx += 1;
							...
							if(c.timer_ment_cnt == 0){
								...
								for(int i=0 ; i<c.getListBoss().size() ; ++i){
									...
									for(int j=0 ; j<count ; ++j){
										...
										if(mi != null){
											c.getListSpawn().add(mi);
											MonsterSpawnlistDatabase.toSpawnMonster(mi, World.get_map(c.getMap()), true, c.getX(), c.getY(), c.getMap(), Lineage.SEARCH_MONSTER_TARGET_LOCATION, 0, true, true);
										}
									}
								}
							}
