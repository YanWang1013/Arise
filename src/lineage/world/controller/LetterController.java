...
public final class LetterController {
	 ...
	 * 있을경우 그에따른 처리를 함께 함.
	 */
	static public void toWorldJoin(Connection con, PcInstance pc){
		boolean find = false;
		...
		try {
			...
			lineage.share.System.printf("%s : toWorldJoin(Connection con, PcInstance pc)\r\n", LetterController.class.toString());
	 ...
	 * 편지지 작성한것을 처리하는 함수.
	 */
	static public void toLetter(object o, String to, String subject, String memo, int aden){
		if(to!=null && subject!=null && memo!=null){
			Connection con = null;
			try {
				con = DatabaseConnection.getLineage();
				toLetter(con, o, "Paper", to, subject, memo, aden);
			} catch (Exception e) {
	...
	static public void toPledgeLetter(Character cha, String to, String subject, String memo){
		if(cha!=null && to!=null && subject!=null && memo!=null){
			// 혈맹 찾기.
			...
			if(clan != null){
				...
				try {
					...
					for(String cm : clan.getMemberList())
						toLetter(con, cha, "clanPaper", cm, subject, memo, 0);
				} catch (Exception e) {
	 ...
	 * @param aden
	 */
	static private void toLetter(Connection con, object o, String type, String to, String subject, String memo, int aden){
		// uid 생성
		...
		if(user != null){
			// 인벤 등록 처리.
			toLetter(user, new_uid, (o==null || o instanceof BoardInstance ? "경매관리자" : o.getName()), to, subject, memo, System.currentTimeMillis(), aden);
			// 비둘기 표현
		...
		// 디비에 등록.
		insert(con, new_uid, type, (o==null || o instanceof BoardInstance ? "경매관리자" : o.getName()), to, subject, memo, String.valueOf(user!=null), aden);
	}
}
