...
public class SlimeRaceController {
	...
	// 슬라임 레이스와 연관된 npc 목록
	static private List<SlimeraceInstance> list_npc;
	// 현재 경기중인 레이서 목록
	static private List<Slimerace> list_race;
	// 골인한 슬라임 목록
	...
	static public void init(Connection con){
		...
		list_npc = new ArrayList<SlimeraceInstance>();
...
//		status = EVENT_STATUS.CLEAR;
		// 경기할 레이서들 객체 미리 생성.
		list_race = new ArrayList<Slimerace>();
		for(int i=0 ; i<5 ; ++i){
			Slimerace s = new Slimerace();
	...
	static public void appendNpc(SlimeraceInstance si){
		list_npc.add( si );
	}
	...
	static public List<String> getSlimeStatus(){
		list_html.clear();
		if(status==EVENT_STATUS.CLEAR){
			for(Slimerace s : list_race){
	...
	static private boolean SlimeLucky(Slimerace s){
	...
	static private int getLastUid(Connection con){
		...
		ResultSet rs = null;
		try {
			st = con.prepareStatement("SELECT MAX(uid) FROM slimerace");
			rs = st.executeQuery();
			if(rs.next())
				return rs.getInt(1);
		} catch (Exception e) {
	...
	static private void toMessage(String msg){
		for(SlimeraceInstance si : list_npc)
	...
	static private void initSlimeRaceTicket(){
		for(SlimeraceInstance si : list_npc){
			for(int i=0 ; i<5 ; ++i){
				...
				si.getNpc().getShop_list().get(i).setSlimeraceUid(now_uid);
			}
		}
	}
	...
	static private void insertDB(){
		...
		try {
			...
			st = con.prepareStatement("INSERT INTO slimerace SET race_idx=?, uid=?, price=?");
	 ...
	 * @param idx
	 */
	static public void setCountting(int idx, int count){
