...
public class PetMasterInstance extends object {
	...
	private Npc npc;
	private List<ItemInstance> temp_list;
	private List<DogCollar> temp2_list;
	
	public PetMasterInstance(Npc npc){
		this.npc = npc;
		temp_list = new ArrayList<ItemInstance>();
		temp2_list = new ArrayList<DogCollar>();
	}
	...
	private void toGetFinal(PcInstance pc, ClientBasePacket cbp){
		...
		while(count-->0){
			...
			boolean pet = SummonController.toPet(pc, (DogCollar)pc.getInventory().value(inv_id));
			if( aden && pet ){
				// 아덴 감소
