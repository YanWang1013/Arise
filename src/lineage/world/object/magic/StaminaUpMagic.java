...
public class StaminaUpMagic extends Magic {

	public StaminaUpMagic(Skill skill){
		super(null, skill);
	}
	
	static public BuffInterface clone(BuffInterface bi, Skill skill, int time){
		if(bi == null)
			bi = new StaminaUpMagic(skill);
		bi.setSkill(skill);
		bi.setTime(time);
		return bi;
	}
	...
	public void toBuffStart(object o) {
		...
		if(o instanceof Character){
			...
			cha.setDynamicHp( cha.getDynamicHp()+40 );
			cha.setDynamicTicHp( cha.getDynamicTicHp()+4 );
			cha.toSender(S_CharacterStat.clone(BasePacketPooling.getPool(S_CharacterStat.class), cha));
		}
		if(Common.requestor.equalsIgnoreCase("giro"))
			ChattingController.toChatting(o, "체력 증강 주문서의 효과가 당신을 감쌉니다.", Lineage.CHATTING_MODE_MESSAGE);
	}
