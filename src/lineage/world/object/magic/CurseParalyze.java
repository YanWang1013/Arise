...
public class CurseParalyze extends Magic {
	...
	static public void init(MonsterInstance mi, object o, MonsterSkill ms, int action, int effect, boolean check){
		...
		if(check && !SkillController.isMagic(mi, ms, true))
			return;
		if(SkillController.isFigure(mi, o, ms.getSkill(), true, check) && !o.isLock()){
			if(effect > 0)
			...
			// 처리.
			BuffController.append(o, CurseParalyze.clone(BuffController.getPool(CurseParalyze.class), ms.getSkill(), ms.getBuffDuration()));
		}
	}
	...
	static public void onBuff(Character cha, object o, Skill skill, int counter){
		...
		if(SkillController.isFigure(cha, o, skill, true, false) && !o.isLock()){
			...
			BuffController.append(o, CurseParalyze.clone(BuffController.getPool(CurseParalyze.class), skill, skill.getBuffDuration()));
			return;
		}
		// \f1마법이 실패했습니다.
