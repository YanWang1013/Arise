...
public class FightStrengthenMagic extends Magic {

	public FightStrengthenMagic(Skill skill){
		super(null, skill);
	}
	
	static public BuffInterface clone(BuffInterface bi, Skill skill, int time){
		if(bi == null)
			bi = new FightStrengthenMagic(skill);
		bi.setSkill(skill);
		bi.setTime(time);
		return bi;
	}
	...
	public void toBuffStart(object o) {
		...
		if(Common.requestor.equalsIgnoreCase("giro"))
			ChattingController.toChatting(o, "전투 강화 주문서의 효과가 당신을 감쌉니다.", Lineage.CHATTING_MODE_MESSAGE);
	}
