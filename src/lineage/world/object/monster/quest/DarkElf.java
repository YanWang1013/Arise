...
public class DarkElf extends MonsterInstance {
	...
	static public void toAttack(MonsterInstance mi, object o, int x, int y, boolean bow){
		if( SkillController.isHpMpCheck(mi, skill) && Util.random(0, 5)==0 ){
			// 액션 취하는 딜레이 주기.
			mi.setAiTime( SpriteFrameDatabase.find(mi.getGfx(), mi.getGfxMode()+Lineage.GFX_MODE_SPELL_DIRECTION) );
			// 마법 시전.
			Tornado.init(mi, skill);
		}else{
			if(Util.isDistance(mi, o, mi.getMonster().getAtkRange())){
				mi.toAttack(o, x, y, bow, mi.getGfxMode()+Lineage.GFX_MODE_ATTACK, 0);
			}else{
				mi.setAiTime( SpriteFrameDatabase.find(mi.getGfx(), mi.getGfxMode()+Lineage.GFX_MODE_WALK) );
