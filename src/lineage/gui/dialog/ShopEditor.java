...
//		step3(null, true);
	...
	static private void step2(String[] shop_list){
		// 이전 내용들 다 제거.
		for(Control c : composite_controller.getChildren())
			c.dispose();
		...
		// 정보 갱신
		int idx = 0;
		for(String s : shop_list){
			Shop shop = npc.findShopItemId(s);
			String[] field = new String[11];
			
			field[0] = String.valueOf(++idx);
			field[1] = npc.getName();
			field[2] = s;
			field[3] = shop==null ? "1" : String.valueOf(shop.getItemCount());
			field[4] = shop==null ? "1" : String.valueOf(shop.getItemBress());
			field[5] = shop==null ? "0" : String.valueOf(shop.getItemEnLevel());
			field[6] = shop==null ? "true" : String.valueOf(shop.isItemSell());
			field[7] = shop==null ? "true" : String.valueOf(shop.isItemBuy());
			field[8] = shop==null ? "false" : String.valueOf(shop.isGamble());
			field[9] = shop==null ? "0" : String.valueOf(shop.getPrice());
			field[10] = shop==null ? "아데나" : shop.getAdenType();
	...
	static private void step3(Table table, boolean db){
		...
		for(TableItem ti : table.getItems()){
			...
			s.setItemSell( Boolean.valueOf(ti.getText(6)) );
			s.setItemBuy( Boolean.valueOf(ti.getText(7)) );
			s.setGamble( Boolean.valueOf(ti.getText(8)) );
			s.setPrice( Integer.valueOf(ti.getText(9)) );
			s.setAdenType( ti.getText(10) );
