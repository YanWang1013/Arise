...
//		step2(null, null);
		...
		button_2.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if(list_1.getSelectionCount() <= 0)
					return;
				// 삭제
				list_1.setData(String.valueOf(list_1.getSelectionIndex()), null);
	...
	static private void step2(String[] inv_list, Map<Integer, Object> list){
		...
		for(String s : inv_list){
			...
			field[15] = String.valueOf(ii==null ? "" : ii.getSlimeRaceTicket());
	...
	static private void step3(Table table){
		...
		for(TableItem ti : table.getItems()){
			...
			String slimerace = ti.getText(15);
	...
	//			ii.setEquipped(equipped);
				...
				ii.setSlimeRaceTicket(slimerace);
				if(item_objectid > 0){
					pc.getInventory().count(ii, ii.getCount(), true);
				}else{
					pc.getInventory().append(ii, true);
					// \f1%0%s 당신에게 %1%o 주었습니다.
					pc.toSender(S_Message.clone(BasePacketPooling.getPool(S_Message.class), 143, "운영자", ii.toString()));
				}
			}
		}
