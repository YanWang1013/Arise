...
//		step4(false, title, title, false, map, map, map, map, map, map, false, title, map, title, map, title, map, title, map, false);
	...
	static private void step3(
				...
				final String monster_3, final int monster_3_count, final String monster_4, final int monster_4_count, final boolean summon
			){
		// 이전 내용들 다 제거.
		for(Control c : composite_controller.getChildren())
			c.dispose();
		...
		button_4.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				// 다음
				step4(db, name, monster, random, count, loc_size, x, y, map, re_spawn, groups, monster_1, monster_1_count, monster_2, monster_2_count, monster_3, monster_3_count, monster_4, monster_4_count, summon);
			}
		});
