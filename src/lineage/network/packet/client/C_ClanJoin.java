...
public class C_ClanJoin extends ClientBasePacket {
	
	static private List<object> list;
	
	static public BasePacket clone(BasePacket bp, byte[] data, int length){
		if(bp == null)
			bp = new C_ClanJoin(data, length);
		else
			((C_ClanJoin)bp).clone(data, length);
		return bp;
	}
	
	public C_ClanJoin(byte[] data, int length){
		clone(data, length);
	...
	public BasePacket init(PcInstance pc){
		...
		if(use!=null && (pc.getGm()>=Lineage.GMCODE || !pc.isTransparent()))
			ClanController.toJoin(pc, use);
		return this;
	}
	 ...
	 * 맞은편에 있는 사용자 찾아서 리턴.
	 */
	static public PcInstance getPlayer(PcInstance pc){
		int locx = pc.getX();
		int locy = pc.getY();
