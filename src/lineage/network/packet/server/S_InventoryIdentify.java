...
public class S_InventoryIdentify extends ServerBasePacket {
	...
	static public BasePacket clone(BasePacket bp, ItemInstance item, String type, int msg){
		if(bp == null)
	...
	public void clone(ItemInstance item, String type, int msg){
		...
		if(type.equalsIgnoreCase("weapon")){
			...
			writeC(0x05);		// 문자열 갯수
			writeS(item.getName());
			writeS(String.valueOf(item.getItem().getDmgMin()));
			writeS(String.valueOf(item.getItem().getDmgMax()));
